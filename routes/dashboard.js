...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sc-info .ow { font-size: 11px; color: var(--t3); margin-top: 1px; }
...
<body>
        ...
        <div class="grid" id="serverGrid">
            <% if (guilds && guilds.length > 0) { %>
                <% guilds.forEach(guild => { %>
                    <div class="sc" data-name="<%= guild.name.toLowerCase() %>">
                        ...
                        <div class="sc-acts">
                            <button class="btn-inv" onclick="inviteBot('<%= guild.id %>')">
                                <span>Invite bot</span>
                            </button>
                            <button class="btn-cfg">
                                <a href="/servers/<%= guild.id %>">Configure</a>
                            </button>
                        </div>
                    </div>
                <% }) %>
            <% } else { %>
                <div class="empty-state">
                    ...
                    <p>You don't have administrator permissions on any servers where the bot is installed.</p>
                </div>
            <% } %>
        </div>
    ...
    <script>
        ...
        function inviteBot(guildId) {
            const inviteUrl = `https://discord.com/oauth2/authorize?client_id=<%= process.env.DISCORD_CLIENT_ID %>&permissions=8&scope=bot%20applications.commands&guild_id=${guildId}`;

...
client.on(Events.InteractionCreate, async (interaction) => {
    if (interaction.isChatInputCommand()) {
        ...
        try {
            ...
            if (interaction.replied || interaction.deferred)
                await interaction.followUp({ content: "❌ Error: " + e.message, ephemeral: true }).catch(() => null);
            else
                await interaction.reply({ content: "❌ Error: " + e.message, ephemeral: true }).catch(() => null);
        }
        // analytics
        if (interaction.guildId)
            await prisma.analytics.upsert({ where: { guildId_date: { guildId: interaction.guildId, date: new Date(new Date().toISOString().slice(0, 10)) } }, update: { commands: { increment: 1 } }, create: { guildId: interaction.guildId, date: new Date(new Date().toISOString().slice(0, 10)), commands: 1 } }).catch(() => null);
        return;
    }
    if (interaction.isButton()) {
        if (interaction.customId === "create_ticket") {
            ...
            const ch = await guild.channels.create({ name: `ticket-${interaction.user.username}`, type: 0, permissionOverwrites: [{ id: guild.roles.everyone, deny: ["ViewChannel"] }, { id: interaction.user.id, allow: ["ViewChannel", "SendMessages"] }] }).catch(() => null);
            if (!ch) {
...
client.on(Events.GuildMemberAdd, async (member) => {
    ...
    if (welcome?.enabled && welcome.channelId) {
        ...
        if (ch) {
            ...
            if (welcome.embedEnabled)
                embeds = [new EmbedBuilder().setTitle(welcome.embedTitle || "Welcome").setDescription(replaceVars(welcome.embedDescription || "", { user: member.user.username, server: member.guild.name })).setColor(parseInt((welcome.embedColor || "#5865F2").replace("#", ""), 16)).setThumbnail(member.user.displayAvatarURL())];
    ...
    if (ar?.enabled && ar.roleId) {
        ...
        if (role && botHighest && role.position < botHighest.position)
    ...
    if (logging?.enabled && logging.logMemberJoin && logChId) {
        ...
        if (ch)
            await ch.send({ embeds: [new EmbedBuilder().setTitle("Member Joined").setDescription(`${member.user.tag} (<@${member.id}>)`).setColor(0x57F287).setThumbnail(member.user.displayAvatarURL()).setTimestamp()] }).catch(() => null);
    }
});
client.on(Events.GuildMemberRemove, async (member) => {
    ...
    if (goodbye?.enabled && goodbye.channelId) {
        ...
        if (ch) {
            let content = replaceVars(goodbye.message || "Goodbye {username}", { user: member.user.tag, username: member.user.username, userid: member.id, server: member.guild.name, membercount: String(member.guild.memberCount) });
...
client.on(Events.MessageCreate, async (msg) => {
    ...
    const last = global.cooldowns.get(key);
    const now = Date.now();
    ...
    if (!last || now - last > cooldown) {
        ...
        if (!lvlSettings?.ignoredChannelIds?.includes(msg.channelId) && !msg.member?.roles.cache.some((r) => lvlSettings?.ignoredRoleIds?.includes(r.id))) {
            ...
            if (newLevel !== current?.level) {
                ...
                if (roleId)
                    await msg.member?.roles.add(roleId).catch(() => null);
            }
        }
    }
    // automod
    ...
    for (const rule of rules) {
        ...
        if (rule.type === "antiLink" && cfg?.enabled !== false) {
            ...
            if (hasLink || (cfg.blockInvite && isInvite)) {
                const allowed = (cfg.allowedDomains || []).some((d) => msg.content.includes(d));
                if (!allowed) {
                    await msg.delete().catch(() => null);
                    await msg.channel.send({ embeds: [new EmbedBuilder().setDescription(`❌ Links not allowed ${msg.author}`).setColor(0xED4245)] }).catch(() => null);
                    break;
                }
            }
        }
    ...
    if (msg.content.startsWith("!")) {
        ...
        if (cc?.enabled) {
            ...
            let embeds = undefined;
            if (cc.embedEnabled)
                embeds = [new EmbedBuilder().setTitle(cc.embedTitle || "").setDescription(cc.embedDescription || content).setColor(parseInt((cc.embedColor || "#5865F2").replace("#", ""), 16))];
...
client.on(Events.MessageDelete, async (msg) => {
    ...
    if (logging?.enabled && logging.logMessageDelete && chId) {
        ...
        if (ch)
            await ch.send({ embeds: [new EmbedBuilder().setTitle("Message Deleted").setDescription(`**Author:** ${msg.author?.tag}\n**Channel:** <#${msg.channelId}>\n**Content:** ${msg.content || "No content"}`).setColor(0xED4245).setTimestamp()] }).catch(() => null);
    }
});
...
client.on(Events.GuildRoleCreate, async (role) => {
    ...
    if (logging?.enabled && logging.logRoleChanges && chId) {
        ...
        if (ch)
            await ch.send({ embeds: [new EmbedBuilder().setTitle("Role Created").setDescription(`${role.name} (${role.id})`).setColor(0x57F287)] }).catch(() => null);
    }
});
...
client.on(Events.VoiceStateUpdate, async (oldS, newS) => {
    ...
    if (logging?.enabled && logging.logVoice && logging.voiceLogChannelId) {
        ...
        if (ch)
            await ch.send({ embeds: [new EmbedBuilder().setTitle("Voice Activity").setDescription(`<@${newS.id}> ${oldS.channelId ? `moved from <#${oldS.channelId}>` : "joined"} to ${newS.channelId ? `<#${newS.channelId}>` : "left"}`).setColor(0x5865F2)] }).catch(() => null);
    }
});
// graceful shutdown
